...
//    System.out.println("Draw Offset: "+ (this.drawOffset));
  ...
  public void remove(int row1, int card1, int row2, int card2) throws IllegalStateException {
    ...
    if (!isValidMove(row1, card1) || !isValidMove(row2, card2)) throw new IllegalArgumentException();
  ...
  public void remove(int row, int card) throws IllegalStateException {
    if (getNumRows() == -1) throw new IllegalStateException(); // game has not started yet
    if (! isValidMove(row, card)) throw new IllegalArgumentException();
    if (((Card) this.pyramid.get(row).get(card)).value != 13) throw new IllegalArgumentException(); 
  ...
  public void removeUsingDraw(int drawIndex, int row, int card) throws IllegalStateException{
    if (getNumRows() == -1) throw new IllegalStateException(); // game has not started yet
    if (! (drawIndex < getDrawCards().size())) throw new IllegalArgumentException(); // invalid draw index
    if (! isValidMove(row, card)) throw new IllegalArgumentException();
  ...
  public boolean isGameOver() throws IllegalStateException {
    if (getNumRows() == -1) throw new IllegalStateException(); // game has not started yet
    /* check if any valid move can be made. If not then, end game
     * a valid move can be made if there is one or two cards in the pyramid (or one in the pyramid and one draw) with a sum of 13
     * Look in pyramid for a card of value 13
     */
  ...
  public K getCardAt(int row, int card) throws IllegalStateException {
    if (getNumRows() == -1) throw new IllegalStateException(); // game has not started yet
    if (! (row < getNumRows()) || row < 0) throw new IllegalArgumentException(); // row number is invalid
    if (! (card < this.pyramid.get(row).size()) || card < 0)throw new IllegalArgumentException(); // column number is invalid
