  ...
  public String format(String template) {
    ...
    while (i < template.length()) {
      if (template.charAt(i) == '%') {
        ...
        case 't':
          temp = String.format("%d", this.inHours());
          break;
        case 'w':
          temp = String.format("%d", AbstractDuration.weeksOf(this.inHours()));
          break;
        case 'W':
          temp = String.format("%02d", AbstractDuration.weeksOf(this.inHours()));
          break;
        case 'd':
          temp = String.format("%d", AbstractDuration.daysOf(this.inHours()));
          break;
        case 'D':
          temp = String.format("%02d", AbstractDuration.daysOf(this.inHours()));
          break;
        case 'h':
          temp = String.format("%d", AbstractDuration.hoursOf(this.inHours()));
          break;
        case 'H':
          temp = String.format("%02d", AbstractDuration.hoursOf(this.inHours()));
          break;
        case '%':
          temp = "%";
          break;
        ...
        default:
          throw new IllegalArgumentException("template malformed");
        }
        str = str + temp;
        i = i + 2;
      }
      else {
        str = str + template.charAt(i);
        i++;
      }
    }
    return str;
